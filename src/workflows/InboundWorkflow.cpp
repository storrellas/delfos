...
we::InboundWorkflow::InboundWorkflow(
...
{
  set_workflow_name(INBOUND_WORKFLOW);
}
...
void we::InboundWorkflow::sequence() {
  BLOG_TRACE_STARTED();
  ...
  try{
    ...
    std::thread alive_thread( &InboundWorkflow::alive_worker, this, std::ref(alive_can_continue));
    ...
    while (true) {
      ...
      //const string iv_phone = service.get_vagroup_phone_significant();
      const string inbound_phone = service.get_visible_phone().get_e164();
      _lead_customer_phone = generate_random_phone();
      _twilio_mockup->request_call_inbound(_lead_customer_phone, inbound_phone);
...
void
we::InboundWorkflow::alive_worker(const std::atomic<bool>& alive_can_continue) {
  BLOG_TRACE_STARTED();
  ...
  try{
    ...
    while (true) {
      ...
      if( not lead_guid_local.empty() )
        alive_gari(lead_guid_local);
...
bool we::InboundWorkflow::twilio_event(const WHttp http_request){
  ...
  if(event_type == wc::twilio_event_type::call){
    ...
    if( calling_from == _lead_customer_phone){
      ...
      size_t init_index = url.find("/ses_") +1;
      size_t end_index = url.find("/ch-agt_") -1 ;
    ...
    if(http_request.get_path().find(twilio_user_session) != string::npos){
...
bool we::InboundWorkflow::twilio_request(const WHttp http_request, const WHttp){


  const string caller = http_request.get_url_param("caller");
  if(caller == _lead_customer_phone){

    const string callsid = http_request.get_url_param("callsid");
    _call.get_user_channel().call();
    _call.get_user_channel().pbx_ok();
    _call.get_user_channel().set_twilio_session(callsid);
  }
