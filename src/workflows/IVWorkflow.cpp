...
we::IVWorkflow::IVWorkflow(
    ...
    _call(),
    _call_bridged(false),
    _twilio_mockup(twilio_mockup)
{
  set_workflow_name(IV_WORKFLOW);
}
...
void we::IVWorkflow::sequence() {
  ...
  try{
    ...
    std::thread alive_thread( &IVWorkflow::alive_worker, this, std::ref(alive_can_continue));
    ...
    while (true) {
      ...
      request_succeeded = _lm_tester.request_get_availability(branch_id, json_response);
      end_trace(request_succeeded, json_response);
      if (!request_succeeded) {
        BLOG_FATAL(
      ...
      /// IV New pin - Landing
      init_trace(wc::lm_event_type::webuser_iv_new_session);
      request_succeeded = _lm_tester.request_ivnewsession(branch_id, json_response);
      end_trace(request_succeeded, json_response);
      if (!request_succeeded) {
        BLOG_FATAL(
          LogItem( wc::lm_event_type::webuser_iv_new_session,
                    wc::concat( "failed for agent '", agent_guid, "'")));
        break;
      }
      // Capture pin
      ...
      /// Inbound Video Call - Twilio
      _call_bridged.store(false);
      wcm::Service service = _wizard_bucket.get_service();
      const string iv_phone = service.get_iv_ddi().get_e164();
      _lead_customer_phone = generate_random_phone();
      _twilio_mockup->request_call_iv(_lead_customer_phone, iv_phone, pin);

      // Wait for call to be bridged
      ...
      string lead_guid_local = _lead_guid;
      session_lock.unlock();
      ...
      request_succeeded = _lm_tester.request_set_result(lead_guid_local, WORKFLOW_LEAD_RESULT, json_response);
      end_trace(request_succeeded, json_response);
      if (!request_succeeded) {
        BLOG_FATAL(
...
void
we::IVWorkflow::alive_worker(const std::atomic<bool>& alive_can_continue) {
  BLOG_TRACE_STARTED();
  ...
  try{
    ...
    string session_guid_local, agent_guid_local, lead_guid_local, pin_local;
    ...
    while (true) {
      ...
      if( not _lead_guid.empty() )
        alive_gari(_lead_guid);

      /// IV Flag
      if( not pin_local.empty() ){
        init_trace_alive(wc::lm_event_type::webuser_iv_flag);
        request_succeeded = _lm_tester.request_ivflag(pin_local, json_response);
        end_trace_alive(request_succeeded, json_response);
        if (!request_succeeded) {
          BLOG_FATAL(
            LogItem( wc::lm_event_type::webuser_iv_flag,
                wc::concat("failed for agent '", agent_guid_local, "'", json_response.to_str())));
          return;
        }
      }
...
bool we::IVWorkflow::twilio_event(const WHttp http_request){
...
bool we::IVWorkflow::twilio_request(const WHttp http_request, const WHttp http_response){

  const string caller = http_request.get_url_param("caller");
  if(caller == _lead_customer_phone){

    const string body = http_response.get_body();
    size_t init_index = body.find("/ch-usr_") + 1;
    size_t end_index = body.find("/iv_pin_inserted");
    const string session_url = body.substr(init_index, end_index - init_index);
