...
we::CMNWorkflow::CMNWorkflow(
    ...
    _call_bridged(false)
{
  set_workflow_name(CMN_WORKFLOW);
}
...
void we::CMNWorkflow::sequence() {
  ...
  try{
    ...
    std::thread alive_thread( &CMNWorkflow::alive_worker, this, std::ref(alive_can_continue));
    ...
    while (true) {
      ...
      request_succeeded = _lm_tester.request_lead(wc::lead_action_type::call_me_now, branch_id,
      ...
      if(json_response.HasMember(helper_capitalize(XML_TAG_DATA).c_str())
         ...
         if(lead_json.HasMember(LEAD_KEY_STR_LEAD.c_str())
            ...
            _lead_guid = lead_json[LEAD_KEY_STR_LEAD.c_str()].GetString();
         }
      }
      if ( _lead_guid.empty() ){
        BLOG_FATAL(
      ...
      /// Gari Update room
      gari_update_room(local_lead_guid);
...
void
we::CMNWorkflow::alive_worker(const std::atomic<bool>& alive_can_continue) {
  BLOG_TRACE_STARTED();
...
//
bool we::CMNWorkflow::twilio_event(const WHttp http_request){
